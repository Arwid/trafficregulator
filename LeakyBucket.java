...
public class LeakyBucket {
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		LeakyBucketSenderThread sender = new LeakyBucketSenderThread(socketB, addr, 4445, arrivalBuffer);
		LeakyBucketArrivalThread arrival = new LeakyBucketArrivalThread(sender, socketA, pout);
		sender.start();
		arrival.start();
  }
}

class LeakyBucketArrivalThread extends Thread {
	private static final int BUFFER_SIZE = 10000;
	DatagramSocket socket;
	LinkedList<DatagramPacket> arrivalBuffer;
	PrintStream pout;
	LeakyBucketSenderThread sender;
	long lastTime = -1;
	LeakyBucketArrivalThread(LeakyBucketSenderThread sender, DatagramSocket socket, PrintStream pout) {
		this.socket = socket;
		this.pout = pout;
		this.sender = sender;
	}
...
class LeakyBucketSenderThread extends Thread {
	...
	InetAddress addr;
	int port;
	LeakyBucketSenderThread(DatagramSocket socket, InetAddress addr, int port, LinkedList<DatagramPacket> arrivalBuffer) {
		this.socket = socket;
		this.addr = addr;
		this.port = port;
